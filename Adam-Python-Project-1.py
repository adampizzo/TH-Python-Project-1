...
def evaluate_high_score(score, high_score):
    if high_score == 9999:
        ...
        if score < high_score:
            print(
                f"But it looks like you just beat it with your score of: {score}\nGreat Job!\n"
            )
            sleep()
            return score
        elif score == high_score:
            print(
                f"It looks like you tied the high score at: {high_score}.\nSo close!\n"
            )
            sleep()
            return high_score
        else:
...
def get_guess():
    while True:
        try:
            ...
            guess = int(
                guess
            )  # I don't like doing it this way, but I haven't found an answer yet
        ...
        try:
            if guess in history:
                raise AlreadyGuessed(
                    f"{guess} has already been guessed, please try again"
                )
            else:
...
def start_game():
    ...
    round_history = (
        []
    )  # Not really using this yet, but it is keeping track of all of the player guesses.
    while keep_playing:
        ...
        if player_score == 0:
            ...
            sleep()
            print(
                "When you finally guess the number correctly we will let you know how many guesses it took.\n"
            )
            sleep()
